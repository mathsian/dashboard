...
def create_assessment_all(cohort, name, date, scale_name, dbname):
    student_df = pd.DataFrame.from_records(data.get_data("enrolment", "cohort",
        cohort, dbname))
...
def get_weekly_attendance(wb, dbname):
    ...
    data.save_docs(df.reset_index().rename(columns={'REGD_Attendance_Mark':
        'marks', 'REGD_Student_ID': 'student_id'}).to_dict(orient='records'),
        dbname)
...
if __name__ == "__main__":
    ...
    create_assessment_all("2022", "First seven weeks", "2020-10-13", "Expectations", "ada")
