with learners as (
    ...
    left join REMSLive.dbo.STESLearnerEmpStatus
        on STES_Student_ID = STUD_Student_ID
        and STES_ISN = (select top 1 u.stes_ISN from remslive.dbo.STESLearnerEmpStatus as u
                             where u.STES_Student_ID = STUD_Student_ID
                             and u.stes_empid is not null
                             order by u.STES_Year desc, u.stes_ISN desc)
    left join REMSLive.dbo.CMPN_Company_main
        on STES_EmpId = CMPN_Company_Code
    where CMPN_Company_Name = '{{employer}}'
     ),
attendance_marks as (
    select
    student_id
